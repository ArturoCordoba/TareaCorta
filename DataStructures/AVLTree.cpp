...
int AVLTree<T>::getBalanceFactor(AVLNode<T> *node) {
    if(node == nullptr) return -1; //Caso en el que el nodo es nulo
...
void AVLTree<T>::insert(T data) {
    ...
    if(root == nullptr){ //Caso en el que el arbol esta vacio
        root = newNode;
...
AVLNode<T>* AVLTree<T>::remove(T element, AVLNode<T> *node) {
    if(node == nullptr){
        ...
        } else if(node->getLeft() == nullptr){ //Caso en el que el nodo no tiene hijo izquierdo
            node = node->getRight();
        } else if(node->getRight() == nullptr){ //Caso en el que el nodo no tiene hijo izquierdo
            node = node->getLeft();
